...
## Oracle external review (when relevant)

For complex/risky code review (watchdog, worker, ingestor, classifier, dashboard, live-pipeline safety), load and follow:

`project-memory/workflows/oracle-review-workflow.md`

- Default: short `oracle -p "…" --file "…"` (browser defaults in `~/.oracle/config.json`)
- New ChatGPT project chat per consult; `--followup` only within the same review session
- Oracle output is advisory — local tests and repo behavior are source of truth
- Log Oracle consultation outcomes via `add_note` or `pjm note` when findings change implementation
